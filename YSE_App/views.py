...
from .query_annotations import annotate_recent_mag, annotate_recent_magdate
    ...
    if 'sort' in request.GET.keys():
        if request.GET['sort'] == 'last_obs_date' or request.GET['sort'] == '-last_obs_date':
            if request.GET['sort'] == '-last_obs_date':
                ...
                is_descending = False
            transients = annotate_recent_magdate(transients).order_by(
                ('-' if is_descending else '') + 'recent_magdate'
            )
            ...
            transients = annotate_recent_mag(transients, alias='last_mag').order_by(
...
def download_target_list(request, telescope, obs_date):
    ...
    follow_requests = TransientFollowup.objects.filter(
        classical_resource=classical_obs_date[0].resource
    ).filter(
        valid_start__lte=classical_obs_date[0].obs_date
    ).filter(
        valid_stop__gte=classical_obs_date[0].obs_date
    ).select_related('transient')
    follow_requests = annotate_recent_mag(
        follow_requests,
        transient_id_field='transient_id',
    )
    comments_by_followup_id = defaultdict(list)
    for followup_id, comment in Log.objects.filter(
        transient_followup__in=follow_requests
    ).values_list('transient_followup_id', 'comment'):
        comments_by_followup_id[followup_id].append(comment)
    ...
    for f in follow_requests:
        comments = ';'.join(comments_by_followup_id[f.id])
        if f.recent_mag is not None:
            content += "%s  %s %s 2000 mag = %.2f comment = %s\n"%(
                f.transient.name.ljust(20),f.transient.CoordString()[0].replace(':',' '),
                f.transient.CoordString()[1].replace(':',' '),float(f.recent_mag),comments)
...
def download_targets_and_finders(request, telescope, obs_date):
    ...
    for f in follow_requests:
        content += "%s  %s %s 2000 mag = %.2f\n"%(
            f.transient.name.ljust(20),f.transient.CoordString()[0].replace(':',' '),f.transient.CoordString()[1].replace(':',' '),float(f.recent_mag))
...
def transient_detail(request, slug):
    ...
    if len(transient) == 1:
        ...
        followups = list(
            TransientFollowup.objects.filter(transient__pk=transient_id).select_related(
                'status',
                'transient',
        ...
        if followups:
            ...
            tasks_by_followup_id = defaultdict(list)
            for task in TransientObservationTask.objects.filter(
                followup_id__in=followup_ids
            ).select_related('status', 'instrument_config'):
                tasks_by_followup_id[task.followup_id].append(task)
            ...
            for i in range(len(followups)):
                followups[i].observation_set = tasks_by_followup_id[followups[i].id]
                ...
                comment_list = comments_by_followup_id[followups[i].id]
                if len(comment_list): followups[i].comment = '; '.join(comment_list)
                #else:
        #   followups = None
